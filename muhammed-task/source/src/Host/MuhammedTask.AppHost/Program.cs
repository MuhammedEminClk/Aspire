...
IResourceBuilder<ProjectResource> productService = builder
    .AddProject<Projects.MuhammedTask_Services_ProductService_WebApi>(productServiceKey)
...
IResourceBuilder<ProjectResource> categoryService = builder
    .AddProject<Projects.MuhammedTask_Services_CategoryService_WebApi>(categoryServiceKey)
    .WithHttpHealthCheck("/health")
    .WithReference(seq)
    .WithReference(cache)
    .WithReference(rabbitmq)
